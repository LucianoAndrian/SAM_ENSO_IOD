...
for s in seasons:
    ...
    for c_count, c in enumerate(cases):
        ...
        data = pd.read_csv(dates_dir + c + '_vs_sam_' + s + '.txt', sep='\t',
                           parse_dates=['time'])
        ...
        for v_count, v in enumerate(variables_tpp):
            ...
            if climatology_neutro:
                ...
                variable_mean = variable.sel(
                    time=variable.time.dt.year.isin(aux.Neutral)).mean('time')
            else:
            ...
            # by sam sign -----------------------------------------------------#
            for sam, sam_title in zip([sam_pos, sam_neg], ['SAM>0', 'SAM<0']):
                print(sam_title)
                if len(sam)==0:
                    continue

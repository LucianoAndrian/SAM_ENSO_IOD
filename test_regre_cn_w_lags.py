...
def CN_Effect(actor_list, set_series_directo, set_series_dmi_total,
    ...
    for x, x_name in zip([amd], ['amd']):
        ...
        for i in ['dmi', 'n34', 'sam']:
            ...
            print(series_aux.keys())
            #print(f"series_aux: {series_aux.keys()}")

            # Efecto total i --------------------------------------------------#
            i_total = regre(series_aux, True, i)
            ...
            try:
                ...
                i_directo = regre(series_directo_aux, True, i)
...
def pre_regre_ufunc(x, x_lag1, x_lag2, modelo, coef, modo):
    pre_serie = {'c': x}
    ...
    actor_list = {'dmi': dmi3.values, 'n34': n343.values, 'sam': sam3.values,
                  ...
                  'hgt_anom_lag1': x_lag1,
                  'hgt_anom_lag2': x_lag2}
    ...
    if modelo.upper() == 'A1':
        ...
        print('Solo modelo A1 disponible')
    ...
    series_dmi_total['x_lag1'] = actor_list[f'hgt_anom_lag1']
    series_dmi_total['x_lag2'] = actor_list[f'hgt_anom_lag2']

    series_n34_total['x_lag1'] = actor_list[f'hgt_anom_lag1']
    series_n34_total['x_lag2'] = actor_list[f'hgt_anom_lag2']

    series_sam_total['x_lag1'] = actor_list[f'hgt_anom_lag1']
    series_sam_total['x_lag2'] = actor_list[f'hgt_anom_lag2']

    series = {'dmi_total': series_dmi_total,
              'n34_total': series_n34_total,
              'sam_total': series_sam_total}

    series_directo['x_lag1'] = actor_list[f'hgt_anom_lag1']
    series_directo['x_lag2'] = actor_list[f'hgt_anom_lag2']
    ...
    if modo.lower() == 'total':
        series_aux = series[f"{coef}_total"]

        series_aux[f"{coef}_lag1"] = actor_list[f"{coef}_lag1"]
        series_aux[f"{coef}_lag2"] = actor_list[f"{coef}_lag2"]


        #return series[f"{coef}_total"]
        efecto = regre(series_aux, True, coef)

    elif modo.lower() == 'directo':

        series_directo_aux[f"{coef}_lag1"] = actor_list[f"{coef}_lag1"]
        series_directo_aux[f"{coef}_lag2"] = actor_list[f"{coef}_lag2"]
        ...
        try:
            ...
            efecto = regre(series_directo_aux, True, coef)


    return efecto

def compute_regression(variable, v_lag1, v_lag2, modelo, coef, modo):
    coef_dataset = xr.apply_ufunc(
        pre_regre_ufunc,
        variable,
        v_lag1.rename({'time': 'time2'}),
        v_lag2.rename({'time': 'time3'}),
        modelo, coef, modo,
        input_core_dims=[['time'],['time2'],['time3'], [], [], []],
        vectorize=True)
    return coef_dataset
...
hgt_anom2_lag1 = hgt_anom_lag1.sel(lat=slice(-80, 20))
hgt_anom2_lag2 = hgt_anom_lag2.sel(lat=slice(-80, 20))
    ...
    for actor in ['dmi', 'n34', 'sam']:
        for modelo in ['A1']:
            for modo in ['total', 'directo']:
                ...
                efecto = compute_regression(v['var'], hgt_anom2_lag1['var'],
                                            hgt_anom2_lag2['var'], modelo,
                                            actor, modo)

                # plt.imshow(efecto, cmap='RdBu')
                # plt.colorbar()
                # plt.show()
